...
export function ThreadedDiscussion({
  ...
  const renderNode = (node: ThreadNode, depth: number) => {
    ...
    return (
      <div key={node.id} className={depth === 0 ? "relative border-b border-[--color-border] py-3 pr-10 last:border-b-0" : "relative border-l border-[--color-border] py-3 pl-3 pr-10"}>
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              {hasReplies && (
                <button type="button" onClick={() => setExpanded((current) => ({ ...current, [node.id]: !isExpanded }))} className="inline-flex items-center gap-1 text-xs text-[--color-link] hover:text-[--color-accent]">
                  {isExpanded ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
                  {isExpanded ? "收起回复" : `查看 ${node.replies.length} 条回复`}
                </button>
              )}
              {canReply && (
                <button
        ...
        {canDelete && onDelete && (
          <button type="button" onClick={() => onDelete(node.id)} className="absolute right-2 top-1/2 -translate-y-1/2 text-[--color-text-muted] hover:text-[--color-danger]" title="删除">
            <Trash2 size={14} />
          </button>
        )}
        {hasReplies && isExpanded && <div className="ml-8 mt-1 space-y-1">{node.replies.map((reply) => renderNode(reply, depth + 1))}</div>}
      </div>
    )
  }
      ...
      {replyingTo && (
        <form onSubmit={submitReply} className="mt-4 rounded-[--radius-md] border border-[--color-border] bg-[--color-bg-hover] p-3">
          <div className="mb-2 flex items-center justify-between gap-3">
            <p className="truncate text-xs text-[--color-text-muted]">回复 {replyingTo.author.displayName || replyingTo.author.email}</p>
            <button
              type="button"
              onClick={() => {
                setReplyingTo(null)
          ...
          <textarea value={replyText} onChange={(event) => setReplyText(event.target.value)} maxLength={maxLength} rows={2} className="w-full resize-none rounded-[--radius-sm] border border-[--color-border] bg-[--color-bg-surface] px-3 py-2 text-sm outline-none focus:border-[--color-accent]" />
          {replySticker && (
            <div className="mt-2 inline-flex items-center gap-2 rounded border border-[--color-border] bg-[--color-bg-surface] px-2 py-1">
              {replySticker.type === "emoji" ? <span className="text-2xl">{replySticker.emoji}</span> : (
                // eslint-disable-next-line @next/next/no-img-element
