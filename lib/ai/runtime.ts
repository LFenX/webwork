...
import { deriveProviderToolSpecs } from "@/lib/ai/tools/derive"
import { validateToolInput } from "@/lib/ai/tools/define"
import { buildAIToolContext, compactText, createAIToolActor } from "@/lib/ai/tools/context"
import { isStructuredToolResult, resolveUserReference, toolForbidden } from "@/lib/ai/tools/helpers"
...
function buildProviderTools(): ProviderToolSpec[] {
  // Derived from the unified registry (description folding + zod/raw JSON Schema),
  // so provider tools stay in lockstep with tool definitions automatically.
  return deriveProviderToolSpecs(AI_TOOLS_REGISTRY)
}
...
async function executeToolCall(
  ...
  try {
    // Validate model-supplied arguments against the tool's zod schema before
    // running it. Acts as a gate: hard violations (missing/typed-wrong required
    // fields) return a structured invalid_args result the model can recover from,
    // instead of reaching execute. Tools still on a raw schema pass through. The
    // original normalizedInput (with injected targetUserId) is what execute
    // receives — validation does not rewrite it, so behavior is unchanged for
    // valid calls.
    const validation = validateToolInput(tool, normalizedInput)
    const result = validation.success
      ? await (tool.execute as (input: Record<string, unknown>) => Promise<unknown>)({
          ...context,
          ...(normalizedInput ?? {}),
        })
      : toolForbidden(`参数校验未通过：${validation.message}。请按工具参数要求修正后重试。`, "invalid_args")
