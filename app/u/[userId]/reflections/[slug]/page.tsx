...
export function generateMetadata({ params }: { params: Promise<{ userId: string; slug: string }> }) {
  return params.then(({ userId, slug }) => getPublicPostMetadata(userId, "reflections", slug, "公开心得文章"))
}
...
export default async function UserReflectionPostPage({
  ...
  const [level, moduleVisibility] = await Promise.all([
    ...
    getModuleVisibility(ownerId, "reflections"),
  ])
  ...
  if (!allowedTypes.includes("reflections")) notFound()

  const visibilities = visibleTo(level)
  const post = await getPost("reflections", decodeURIComponent(slug), ownerId, visibilities)
  if (!post) notFound()
  ...
  await recordVisit({ ownerId, visitorId: session?.userId ?? null, module: "reflections", path: profileHref(creator, `reflections/${slug}`), postId: post.id })
  ...
  return (
    <ArticleReader
      ...
      backHref={profileHref(creator, "reflections")}
      backLabel={dict.article.backTo(dict.nav.reflections)}
