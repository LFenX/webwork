...
export function generateMetadata({ params }: { params: Promise<{ userId: string; slug: string }> }) {
  return params.then(({ userId, slug }) => getPublicPostMetadata(userId, "notes", slug, "公开笔记"))
}
...
export default async function UserNotePostPage({
  ...
  const [level, moduleVisibility] = await Promise.all([
    ...
    getModuleVisibility(ownerId, "notes"),
  ])
  ...
  await recordVisit({ ownerId, visitorId: session?.userId ?? null, module: "notes", path: profileHref(creator, `notes/${slug}`), postId: post.id })
  ...
  return (
    <ArticleReader
      ...
      backHref={profileHref(creator, "notes")}
