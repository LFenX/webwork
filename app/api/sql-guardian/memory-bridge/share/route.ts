...
import { createSoulWingToGuardianBridge } from "@/lib/sql-guardian/server/bridge-service"
import { GuardianServiceError } from "@/lib/sql-guardian/server/profile-service"
import { createGuardianMemoryBridgeShareSchema } from "@/lib/sql-guardian/server/validation"
...
export async function POST(req: NextRequest) {
  ...
  const parsed = createGuardianMemoryBridgeShareSchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json({ error: "INVALID_REQUEST", detail: parsed.error.flatten() }, { status: 400, headers: NO_STORE })
  }

  try {
    const result = await createSoulWingToGuardianBridge(session.userId, parsed.data)
